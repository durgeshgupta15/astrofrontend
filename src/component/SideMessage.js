export default function Noti({ content, status, animation = true, timer = 4000, progress = true, bgcolor, icon = 'show' }) {
    if (timer > 10000) {
        timer = 4000;
    }
    var status = status;
    ...
    switch (status) {
        case 'success':
            ...
            icon == 'show' || icon == '' ?
                Noti_alert.innerHTML = "<i className='fa fa-check' aria-hidden='true'></i>" + "<span>" + content + "</span>"
                :
        ...
        case 'warning':
            ...
            icon == 'show' || icon == '' ?
                Noti_alert.innerHTML = "<i className='fa fa-exclamation-triangle' aria-hidden='true'></i>" + "<span>" + content + "</span>"
                :
                Noti_alert.innerHTML = content;
            break;
        case 'danger':
